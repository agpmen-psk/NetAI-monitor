...
def _apply_parsed_response(incidents: List[Incident], text: str) -> None:
    ...
    if not parsed_any and clean_text.strip():
        ...
        # показываем сырой ответ целиком вместо пустоты, но НЕ помечаем
        # ai_analyzed=True: раньше это делало провал финальным для целого
        # батча сразу, без единой попытки авторетрая в _analyze_batch — при
        # обычной временной обрезке/сбое формата все инциденты батча
        # намертво получали одинаковый (и не относящийся к ним конкретно)
        # текст. Оставляя ai_analyzed=False, тот же _analyze_batch сначала
        # попробует меньшими пачками; текст ниже — просто что показать,
        # пока идёт повтор, а не окончательный ответ.
        for inc in incidents:
            if not inc.ai_analyzed:
                inc.ai_summary = "Не удалось разобрать формат ответа модели (повтор автоматически):"
...
class OllamaAnalyzer(BaseAnalyzer):
    ...
    def _analyze_batch(self, batch: List[Incident], rag, depth: int = 0) -> None:
        ...
        if len(batch) > 1 and depth < 3:
            ...
            # ai_analyzed остаётся False, чтобы realtime_engine.get_unanalyzed()
            # подхватил и повторил попытку на следующем цикле опроса, а не
    ...
    def _call_incidents(self, prompt: str, system: str, batch: List[Incident]) -> None:
        ...
        response = self._requests.post(
            ...
            # Раньше 300 — с батчами по 10 и без авторетрая это было разумно.
            # Теперь на один вызов analyze() может прийтись несколько
            # последовательных вызовов (авторетрай меньшими пачками, см.
            # _analyze_batch, до глубины 3) — 300с на каждый в худшем случае
            # означал, что realtime_service.py мог не отвечать на Ctrl+C
            # десятки минут. 120с достаточно для пачки из ≤5 коротких
            # инцидентов на локальной модели; если и это не уложилось —
            # быстрее упасть в RuleBasedAnalyzer, чем ждать дальше.
            timeout=120,
        )
